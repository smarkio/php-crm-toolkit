...
class SoapRequestsGenerator
{
    ...
    public static function generateUpsertRequest(Entity $entity)
    {
            ...
            if ($entity->propertyValues[$property]['Changed'])
            {
                ...
                if ($propertyDetails->isLookup)
                {
                    ...
                    if ($entity->propertyValues[$property]['Value'] != null)
                    {
                        ...
                        $valueNode->appendChild($executeActionRequestDOM->createElement('b:Name'))
                            ->setAttribute('i:nil', 'true');
                    }
                    else
                    {
                ...
                {
                    if (strtolower($propertyDetails->type) == "money")
                    {
                    ...
                    {
                        if (strtolower($propertyDetails->type) == "datetime")
                        {
                        ...
                        {
                            if (strtolower($propertyDetails->type) == "picklist")
                            {
                                $valueNode = $propertyNode->appendChild($executeActionRequestDOM->createElement('c:value'));
        ...
        /* Entity State */
        $mainValueNode->appendChild($executeActionRequestDOM->createElement('b:EntityState'))
            ->setAttribute('i:nil', 'true');
        /* Formatted Values */
    ...
    public static function generateRetrieveEntityRequest($entityType, $entityId = null, $entityFilters = null, $showUnpublished = false)
    {
        ...
        /* Request ID and Name */
        $requestNode->appendChild($retrieveEntityRequestDOM->createElement('b:RequestId'))
            ->setAttribute('i:nil', 'true');
            ...
            foreach ($parameters as $parameter)
            {
                ...
                switch ($xmlType)
                {
                    ...
                    case 'entityreference':
                        ...
                        if ($xmlType === 'Entity')
                        {
                            $entityChilds = $entity->getEntityDOM(true)->childNodes;
                            for ($i = 0; $i < $entityChilds->length; $i++)
                            {
                                $entityValue->appendChild($executeActionRequestDOM->importNode($entityChilds->item($i), true));
                            }
                        }
                        else
                        {
                ...
                $valueNode   = !empty($entityValue) ? $propertyNode->appendChild($entityValue) :
